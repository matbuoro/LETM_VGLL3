...
load("/media/HDD12To/mbuoro/LETM/results/RESULTS_vgll3_scorff.RData")
...
#for (j in 1:length(population)){
  ...
  smp<-sample(5000:nrow(etas),10,replace=FALSE)
  ...
  plot(density(X,adjust=2),type='n',xlim=c(-8,8),ylim=c(0,1),xlab='',ylab='')
  mtext("Density",side=2,line=2,at=3,cex=1.5)
  for (i in 1:length(smp)){
    lines(density(smp.thetas[i,which(sex==1 & g==1)]),col=colors[1])
    lines(density(smp.thetas[i,which(sex==1 & g==2)]),col=colors[2])
    lines(density(smp.thetas[i,which(sex==1 & g==3)]),col=colors[3])
  }
  ## Density Threshold
  for (i in 1:3){
    theta <- density(thetas.median[which(sex==1 & g==i)],adjust=2)
    polygon(theta, col=colors[i], border=colors[i])
  }
  #rug(df$theta, col="red")
  ...
  ## Density Proximate cue
  for (i in 1:3){
    eta <- density(etas.median[which(sex==1 & g==i)],adjust=2)
    polygon(eta, col=colors[i], border=colors[i])
  }
  #par(new=TRUE);hist(median.etas, prob=TRUE, col="grey",xlim=c(1,3.5),ylim=c(0,6),breaks=30)
  #plot(density(smp.etas[1,]),col=newcol[2])
  for (i in 1:length(smp)){
    lines(density(smp.etas[i,males]),col=newcol[2])
  }
  #lines(eta,col=2,lwd=3)
