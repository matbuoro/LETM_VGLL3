...
The genotypic values of VGLL3 are similar between sexes: `r format_ci2(a[1])` in males and `r format_ci2(a[2])` in females. However, males exhibit complete dominance of the E allele (k = `r format_ci2(k[,1])`), whereas females show only weak dominance (k = `r format_ci2(k[,2])`, p[k \< 0] = `r mean(k[,2]<0)`).
...
Our results indicate a decline in the proximal signal over time, with a significant decrease after 2005. However, no clear temporal trend was observed for maturation thresholds in either sex. A slight but non-significant negative temporal correlation was observed between maturation thresholds and proximal signals, suggesting a plastic rather than genetic response.
Additionally, we observed a slight decline in the frequency of the L allele over time, with a positive correlation with the maturation threshold in females only.
